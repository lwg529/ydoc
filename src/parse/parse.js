...
// 根据路径，获取判断index是不是md,jsx,html，是则返回文件路径，否则返回空
function getIndexPath(filepath) {
    // 获取type的类型
    let getIndexPathByType = type =>
        path.resolve(filepath, defaultIndexPageName + '.' + type);
    ...
    try {
        ...
        if (!indexPath) {
            return utils.log.error(
                `The root directory of site didn't find index page.`
            );
        }
...
async function parseBook({ bookpath, indexFile, title }) {
    ...
    if (summary && Array.isArray(summary)) {
        parseDocuments(bookpath, function(
            absolutePath,
            releativeHtmlPath,
            title = ''
        ) {
    ...
    let showpath = color.yellow(
        bookpath + '/' + defaultIndexPageName + '.html'
    );
