...
function addTplHook(arr) {
    arr.forEach(
        hookname =>
            (hooks[utils.defaultTplHookPrefix + hookname] = {
                listener: []
            })
    );
}
    ...
    if (hooks[name] && typeof hooks[name] === 'object') {
        ...
        if (Array.isArray(hooks[name].listener)) {
            ...
            for (let i = 0, l = listenerList.length; i < l; i++) {
                ...
                promiseAll.push(
                    Promise.resolve(listenerList[i].fn.apply(context, args))
                );
            }
        }
...
function handleAssets(config, dir, pluginName) {
    ...
    if (config && typeof config === 'object') {
        if (config.dir) {
            ...
            pluginAssetPath = path.resolve(
                ydoc.config.dist,
                'ydoc/ydoc-plugin-' + pluginName
            );
...
exports.loadPlugins = function loadPlugins() {
    ...
    for (let i = 0, l = plugins.length; i < l; i++) {
        ...
        try {
            ...
            if (
                ...
                try {
                    pluginModuleDir = path.resolve(
                        modules,
                        './ydoc-plugin-' + pluginName
                    );
                    pluginModule = require(pluginModuleDir);
                } catch (err) {
                    pluginModuleDir = path.dirname(
                        require.resolve('ydoc-plugin-' + pluginName)
                    );
                    pluginModule = require(pluginModuleDir);
                }
            ...
            let options =
                typeof ydocConfig.pluginsConfig === 'object' &&
            ...
            err.message =
                'Load ' +
                path.resolve(modules, './ydoc-plugin-' + pluginName) +
                ' plugin failed, ' +
                err.message;
            throw err;
        }
