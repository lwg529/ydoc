...
exports.runBatch = async function runBatch() {
    ...
    for (let index = 0, l = batch.length; index < l; index++) {
        ...
        switch (page.type) {
            ...
            case 'jsx':
                ...
                parseJsxInst.data =
                    parseJsxInst.data && typeof parseJsxInst.data === 'object'
                        ? parseJsxInst.data
                        : {};
                _p = {
                    title: parseJsxInst.data.title || '',
                    description: parseJsxInst.data.description || '',
                    content: parseJsxInst.render(
                        Object.assign({}, context, parseJsxInst.data)
                    )
                };
                break;
            default:
    ...
    batch.forEach(transaction => {
        if (
            utils.fileExist(transaction.context.page.srcPath) &&
            transaction.context.page.srcPath !==
                transaction.context.page.distPath
        ) {
